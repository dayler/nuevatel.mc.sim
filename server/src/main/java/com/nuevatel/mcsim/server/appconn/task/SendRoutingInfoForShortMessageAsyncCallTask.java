...
/**
 * Handle <code>SendRoutingInfoForShortMessageAsyncCall</code> message.
 *
 * @author Ariel D. Salazar H.
 *         Created by asalazar on 11/15/15.
 */
public class SendRoutingInfoForShortMessageAsyncCallTask implements Task {

    private static Logger logger = LogManager.getLogger(SendRoutingInfoForShortMessageAsyncCallTask.class);
    ...
    public Message execute(Conn conn, Message message) throws Exception {
        ...
        ServerProcessor processor = AllocatorService.findProcessor(messageId); // messageId is the same of dialog id
        if (processor == null) {
